...
const startScheduler = () => {
  // ⚠️ Change back to '0 8 * * *' after testing
  cron.schedule('* * * * *', async () => {
    console.log('📧 Running daily reminder check...');
    const today = new Date();
    today.setHours(0, 0, 0, 0);

    // 1. Find all items with unsent reminders (for expiring alerts)
    ...
    // 2. Group items by user (expiring matches)
    const userMap = new Map(); // userId string -> { owner, items: [], includedIds: Set }
    const includedItemIds = new Set(); // track IDs already added to avoid duplicates later
    ...
    for (const item of itemsWithUnsentReminders) {
      ...
      if (diffDays > 0) {
        // Expiring soon: find exact match
        matchingReminder = item.reminders.find(
          (r) => !r.sent && r.daysBefore === diffDays
        );
      } else {
        // Already expired (caught by unsent reminders) – include once, then mark all sent
        matchingReminder = item.reminders.find((r) => !r.sent);
      }

      if (!matchingReminder) continue;

      // Mark reminder(s) as sent so they don't fire again
      if (diffDays <= 0) {
        // Expired: mark all unsent reminders as sent
    ...
    // 3. For each user, also fetch ALL expired items (endDate < today) that were not already included
    const allUserIds = Array.from(userMap.keys());

    // Also need to consider users who have expired items but no expiring items – they would not be in userMap yet.
    // We'll collect all distinct user IDs from expired items as well.
    const expiredItems = await Item.find({
      endDate: { $lt: today },
      _id: { $nin: Array.from(includedItemIds) },  // exclude already added
    })
      .populate('userId', 'email name')
      .populate('type', 'name')
      .populate('provider', 'name')
    ...
    // 4. Send one consolidated email per user
    for (const [userId, data] of userMap) {
      ...
      // Save only the items whose reminders were modified (those from the first loop)
      // The expired items fetched separately are not modified, so we don't need to save them.
    ...
    console.log(`✅ Daily reminder check complete – ${userMap.size} user(s) notified.`);
  });
};
